...
public class MovementCalculator {
	...
	// Based on camera geometry detailed in a lecture by professor Silvio Silvarese
	// at Stanford
	// The Z plane is forward and backward, the X plane is left and right, and the Y
	// plane is up and down
	public void calculateRelativeXYZPosition(DoublePointer X, DoublePointer Y, DoublePointer Z, double cameraX,
			double cameraY, double cameraSize, double cameraReserved) {
		double d = CameraConstants.ANGLED_HEIGHT
				* Math.sqrt(sqr(cameraX / cameraY) + sqr(CameraConstants.FOCAL_LENGTH / cameraY) + 1);

		double d0 = d * Math.sin(CameraConstants.THETA_Y_XZ);

		double theta_x_yz = Math.asin((cameraX - CameraConstants.CAMERA_WIDTH / 2) / CameraConstants.CAMERA_WIDTH)
				* CameraConstants.CAMERA_ANGLE_X_YZ_INTERVAL_SIZE + CameraConstants.CAMERA_ANGLE_X_YZ_OFFSET;
